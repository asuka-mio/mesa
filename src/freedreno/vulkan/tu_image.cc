                       ...
                       uint64_t modifier, const VkSubresourceLayout *plane_layouts)
{
   ...
   for (uint32_t i = 0; i < tu6_plane_count(image->vk.format); i++) {
      ...
      uint32_t width0 = image->extent_w;
      uint32_t height0 = image->extent_h;
              ...
              const VkImageCreateInfo *pCreateInfo)
{
   ...
   image->extent_w = pCreateInfo->extent.width;
   image->extent_h = pCreateInfo->extent.height;
   image->extent_d = pCreateInfo->extent.depth;
   image->samples = pCreateInfo->samples;
   image->mipLevels = pCreateInfo->mipLevels;
   image->arrayLayers = pCreateInfo->arrayLayers;
   image->image_type_3d = pCreateInfo->imageType == VK_IMAGE_TYPE_3D;
